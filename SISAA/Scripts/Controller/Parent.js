...
function bindTable(data){
    ...
    $.each(data, function(i, item) {
        ...
        btnRow  = btnRow + '<a class="dropdown-item" href="#" data-toggle="modal" data-target="#EditarEncarregado'+count+'">Editar Encarregado</a>';
        ...
        var $tr = $('<tr>').append(
            $('<td class="d-none">').text(item.parentID),
            $('<td>').text(item.name),
            $('<td>').text(item.profession),
            $('<td>').text(item.address),
            $('<td>').text(item.cellphone),
            $('<td>').html(btnRow)
        );
        console.log($tr);

        $("#parentes tbody").append($tr);
...
function VerEncarregado(idEstudante){
    alert(idEstudante);   
}
